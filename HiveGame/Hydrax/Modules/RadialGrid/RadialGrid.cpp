...
namespace Hydrax{namespace Module
{
	...
	RadialGrid::RadialGrid(Hydrax *h, Noise::Noise *n, const MaterialManager::NormalMode& NormalMode, const Options &Options)
		: Module("RadialGrid" + _RG_getNormalModeString(NormalMode),
		         n, Mesh::Options(0, Size(int(Options.Radius)*2), _RG_getVertexTypeFromNormalMode(NormalMode)), NormalMode)
		, mHydrax(h)
		, mVertices(0)
		, mVerticesChoppyBuffer(0)
	{
	...
	{
		mMeshOptions.MeshSize     = Size(int(Options.Radius)*2);
		...
		if (isCreated())
		{
			...
			if (getNormalMode() == MaterialManager::NM_VERTEX)
			{
				...
				float r_scale = mOptions.Radius/
					...
					mOptions.StepSizeCube * Ogre::Math::Pow(Ogre::Real(mOptions.Circles), 3) + 
				     mOptions.StepSizeFive * Ogre::Math::Pow(Ogre::Real(mOptions.Circles), 5));

				for(y=0;y<mOptions.Circles;y++) 
				{
					float r = r_scale*(mOptions.StepSizeLin * (y+1) + mOptions.StepSizeCube * Ogre::Math::Pow(y+1.f, 3) + mOptions.StepSizeFive * Ogre::Math::Pow(y+1.f, 5));
