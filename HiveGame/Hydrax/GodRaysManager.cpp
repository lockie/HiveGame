...
namespace Hydrax
{
	...
	void GodRaysManager::create(const HydraxComponent& HC)
	{
		...
		mPerlin = new Noise::Perlin(Noise::Perlin::Options(8, 0.085f, 0.49f, 2, 0.672f));
		mPerlin->create();
	...
	Ogre::Vector2 GodRaysManager::_calculateRayPosition(const int& RayNumber)
	{
		...
		Ogre::Vector2 RayPos = 
			Ogre::Vector2( // X coord
			              Ogre::Real(static_cast<int>(XCoord)),
						  // Y coord
						  Ogre::Real(static_cast<int>((RayNumber+sqrt_NumberOfRays)/sqrt_NumberOfRays)-1));
