...
namespace Caelum
{
    ...
    const Ogre::Degree PointStarfield::DEFAULT_OBSERVER_POSITION_REBUILD_DELTA = Ogre::Degree(0.1f);
	...
	void PointStarfield::addRandomStars (int count)
	{
		for (int i = 0; i < count; ++i) {
			...
			Star s;
			s.RightAscension = Ogre::Degree (Ogre::Real(rasc));
			s.Declination = Ogre::Degree (Ogre::Real(decl));
			// This distribution is wrong.
			s.Magnitude = 6 * pos.squaredLength () + 1.5f;
			mStars.push_back(s);
		}
		notifyStarVectorChanged ();
	}
	...
	void PointStarfield::ensureGeometry ()
	{
        ...
        for (uint i = 0; i < starCount; ++i)
        {
    		...
    		Ogre::Vector3 pos;
			pos.z = -Math::Cos (Ogre::Degree(Ogre::Real(azm))) * Math::Cos (Ogre::Degree(Ogre::Real(alt)));
		    pos.x =  Math::Sin (Ogre::Degree(Ogre::Real(azm))) * Math::Cos (Ogre::Degree(Ogre::Real(alt)));
		    pos.y = -Math::Sin (Ogre::Degree(Ogre::Real(alt)));
