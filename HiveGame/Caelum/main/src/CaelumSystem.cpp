...
namespace Caelum
{
    ...
    )
    {
        ...
        if (componentsToCreate & CAELUM_COMPONENT_SUN) {
            try {
                ...
                this->getSun ()->setAmbientMultiplier (Ogre::ColourValue (0.5f, 0.5f, 0.5f));
                this->getSun ()->setDiffuseMultiplier (Ogre::ColourValue (3, 3, 2.7f));
                this->getSun ()->setSpecularMultiplier (Ogre::ColourValue (5, 5, 5));

                this->getSun ()->setAutoDisable (true);
                this->getSun ()->setAutoDisableThreshold (0.05f);
            } catch (Caelum::UnsupportedException& ex) {
                LogManager::getSingleton ().logMessage (
        ...
        if (componentsToCreate & CAELUM_COMPONENT_MOON) {
            try {
                ...
                this->getMoon ()->setAutoDisableThreshold (0.05f);
            } catch (Caelum::UnsupportedException& ex) {
                LogManager::getSingleton ().logMessage (
        ...
        if (componentsToCreate & CAELUM_COMPONENT_CLOUDS) {
            try {
                ...
                getCloudSystem ()->getLayer (0)->setCloudCover (0.3f);
            } catch (Caelum::UnsupportedException& ex) {
                LogManager::getSingleton ().logMessage (
    ...
    void CaelumSystem::updateSubcomponents (Real timeSinceLastFrame)
    {
        ...
        */
		...
		Real fogDensity = getFogDensity (Ogre::Real(relDayTime), sunDir);           
        Ogre::ColourValue fogColour = getFogColour (Ogre::Real(relDayTime), sunDir);                  
        Ogre::ColourValue sunLightColour = getSunLightColour (Ogre::Real(relDayTime), sunDir);
        Ogre::ColourValue sunSphereColour = getSunSphereColour (Ogre::Real(relDayTime), sunDir);
        ...
        if (getImageStarfield ()) {
            getImageStarfield ()->update (Ogre::Real(relDayTime));
        ...
        if (getPointStarfield ()) {
            ...
            getPointStarfield ()->_update (Ogre::Real(relDayTime));
        }
    ...
    Ogre::ColourValue CaelumSystem::getFogColour (Real time, const Ogre::Vector3 &sunDir) {
        ...
        Real elevation = sunDir.dotProduct (Ogre::Vector3::UNIT_Y) * 0.5f + 0.5f;
